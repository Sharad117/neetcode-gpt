...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.word_emb=nn.Embedding(vocab_size,model_dim)
        self.position_emb=nn.Embedding(context_length,model_dim)
        self.block=nn.Sequential(*[self.TransformerBlock(model_dim,num_heads) for _ in range(num_blocks)])
        self.final_layer_norm=nn.LayerNorm(model_dim)
        self.ffn=nn.Linear(model_dim,vocab_size)
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        embedded=self.word_emb(context)+self.position_emb(positions)
        embedded=self.block(embedded)
        out=self.final_layer_norm(embedded)
        logits=self.ffn(out)
